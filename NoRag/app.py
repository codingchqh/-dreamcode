...
def initialize_session_state():
    st.session_state.dream_text = "" # 전사 결과 텍스트 (이제 원본 텍스트는 original_dream_text에)
    st.session_state.original_dream_text = "" # 초기화
    st.session_state.derisked_text = ""
    st.session_state.dream_report = None
    st.session_state.nightmare_prompt = ""
    st.session_state.reconstructed_prompt = ""
...
if audio_bytes is not None and not st.session_state.audio_processed:
    ...
    with st.spinner("음성을 텍스트로 변환하고 안전성 검사 중... 🕵️‍♂️"):
        ...
        if safety_result["flagged"]:
            st.error(safety_result["text"]) # 안전성 검사 메시지는 여기서만 표시
            st.session_state.audio_processed = False
            st.session_state.dream_text = "" # 플래그 되면 dream_text 비움
        else:
            st.session_state.dream_text = transcribed_text # 실제 꿈 텍스트 저장
            st.success("안전성 검사: " + safety_result["text"]) # 안전하다는 메시지 표시
            st.session_state.audio_processed = True

    os.remove(audio_path)
    st.rerun()

# --- 9. 2단계: 텍스트 출력 및 분석 시작 버튼 ---
# 이제 st.session_state.original_dream_text를 사용하여 원본 텍스트를 표시
if st.session_state.original_dream_text: 
    ...
    # 안전성 검사에서 문제가 없어야 분석 시작 버튼이 활성화됨
...
if st.session_state.analysis_started and st.session_state.dream_report is None:
    if st.session_state.original_dream_text: # 원본 텍스트가 있어야 리포트 생성 시도
        ...
        st.session_state.analysis_started = False # 분석 상태 초기화 (오류 방지)
...
if st.session_state.dream_report:
    ...
    if emotions:
        ...
        for emotion in emotions:
            ...
            score = emotion.get('score', 0) # score는 0~1.0 사이 값
            st.progress(score, text=f"{score*100:.1f}%") # 백분율로 표시
    ...
    with col1:
        if st.button("😱 악몽 이미지 그대로 보기"):
            with st.spinner("악몽을 시각화하는 중... 잠시만 기다려주세요."):
                # original_dream_text를 사용하여 프롬프트 생성
                ...
                # 재구성 이미지 관련 세션 상태 초기화 (악몽 이미지를 먼저 볼 때)
                st.session_state.reconstructed_prompt = ""
                st.session_state.transformation_summary = ""
                st.session_state.keyword_mappings = []
                st.session_state.reconstructed_image_url = ""
    ...
    with col2:
        if st.button("✨ 재구성된 꿈 이미지 보기"):
            with st.spinner("악몽을 긍정적인 꿈으로 재구성하는 중... 🌈"):
                # original_dream_text와 dream_report를 사용하여 프롬프트 생성
                reconstructed_prompt, transformation_summary, keyword_mappings = \
                    _dream_analyzer_service.create_reconstructed_prompt(
                        st.session_state.original_dream_text, # 원본 텍스트 전달
                        st.session_state.dream_report         # 감정 분석 리포트 전달
                    )
                ...
                # 악몽 이미지 관련 세션 상태 초기화 (재구성 이미지를 먼저 볼 때)
                st.session_state.nightmare_prompt = ""
                st.session_state.nightmare_image_url = ""
