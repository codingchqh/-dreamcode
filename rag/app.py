...
def run_analysis_pipeline(dream_text):
    ...
    st.session_state.show_before_image = False
    st.session_state.show_after_image = False
...
def main():
    ...
    # 세션 상태 초기화: 'dream_text'를 단일 정보 소스로 사용
    ...
    st.divider()
    # 중앙 분석 버튼: 이 버튼은 오직 위 텍스트 상자의 내용만 신경 씀
    if st.button("분석 및 재구성 시작하기", type="primary", use_container_width=True):
        # 버튼을 눌렀을 때, 현재 텍스트 상자의 값을 가져와서 파이프라인 실행
