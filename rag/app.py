...
def initialize_services():
    ...
    try:
        ...
        st.info("faiss_index 폴더를 확인해주세요.")
        st.stop()
...
def main():
    ...
    with tab1:
        ...
        if st.button("분석 시작 (텍스트)", type="primary", use_container_width=True):
            st.session_state.transcribed_text = text_input; run_analysis_pipeline(st.session_state.transcribed_text)
    with tab2:
        st.file_uploader("음성 파일(mp3, wav, m4a 등)을 업로드하세요.", type=['mp3', 'm4a', 'wav', 'ogg'], key="file_uploader", on_change=handle_file_upload)
        if st.session_state.transcribed_text:
            st.text_area("**변환된 텍스트** (수정 가능)", value=st.session_state.transcribed_text, height=150, key="transcribed_text_area")
            if st.button("분석 시작 (업로드 파일)", type="primary", use_container_width=True): run_analysis_pipeline(st.session_state.transcribed_text)
    with tab3:
        ...
        if st.session_state.transcribed_text:
            ...
            if st.button("분석 시작 (녹음 내용)", type="primary", use_container_width=True): run_analysis_pipeline(st.session_state.transcribed_text)
