...
def run_analysis_pipeline(dream_text):
    ...
    st.divider()
    st.subheader("✨ 이렇게 바뀌었어요!"); st.write(summary)
...
def main():
    ...
    # --- 🔽 여기가 핵심 변경 사항 🔽 ---
    # 세션 상태에 'transcribed_text'가 없으면 빈 문자열로 초기화
    ...
    with tab2:
        ...
        if uploaded_file is not None:
            if st.button("이 파일로 텍스트 변환하기", use_container_width=True):
    ...
    with tab3:
        ...
        if webrtc_ctx.audio_processor:
            if st.button("녹음 내용 텍스트로 변환", use_container_width=True):
