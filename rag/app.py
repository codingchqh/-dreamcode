...
def main():
    ...
    with st.expander("음성으로 입력하기 (파일 업로드 또는 실시간 녹음)"):
        ...
        with col_upload:
            ...
            # key와 on_change를 사용하여 파일 업로드 시 콜백 함수가 실행되도록 설정
            st.file_uploader("음성 파일 업로드", type=['mp3', 'm4a', 'wav', 'ogg'], key="file_uploader", on_change=transcribe_audio_on_change, label_visibility="collapsed")
        with col_record:
            st.markdown("##### 🎤 실시간 녹음")
            webrtc_ctx = webrtc_streamer(key="audio-recorder", mode=WebRtcMode.SENDONLY, audio_processor_factory=AudioFrameHandler, media_stream_constraints={"video": False, "audio": True})
    ...
    # 중앙 텍스트 입력 영역: 모든 입력의 결과가 이곳으로 모임
    # 사용자가 직접 수정할 수도 있고, 콜백 함수에 의해 채워질 수도 있음
